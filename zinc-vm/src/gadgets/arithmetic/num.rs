...
impl<F: Field> AllocatedNum<F> {
    ...
    {
        let value = value();
        let var = cs.alloc_input(|| "input num", || value)?;
    ...
    pub fn pow<CS>(&self, mut cs: CS, power: &F) -> Result<Self, SynthesisError>
    where
        CS: ConstraintSystem<F>,
    {
        let power_bits: Vec<bool> = BitIterator::new(power.into_repr()).collect();
        let mut temp = AllocatedNum::alloc(cs.ns(|| "one"), || Ok(F::one()))?;
        temp.assert_number(cs.ns(|| "assert_one"), &F::one())?;
    ...
    pub fn pack_bits_to_element<CS: ConstraintSystem<F>>(
        mut cs: CS,
        bits: &[Boolean],
    ) -> Result<Self, SynthesisError> {
    ...
    */
    ...
    pub fn assert_number<CS>(&self, mut cs: CS, number: &F) -> Result<(), SynthesisError>
    where
        CS: ConstraintSystem<F>,
    {
        cs.enforce(
            || "number assertion constraint",
            |zero| zero + self.var - (number.clone(), CS::one()),
            |zero| zero + CS::one(),
            |zero| zero,
        );
        ...
        where CS: ConstraintSystem<F>
    {
        ...
        cs.enforce(
            ...
            |zero| zero + a.var - b.var,
            |_| condition.lc(CS::one(), F::one()),
            |zero| zero + a.var - c.var
        );
        ...
        cs.enforce(
            ...
            |zero| zero + b.var - a.var,
            |_| condition.lc(CS::one(), F::one()),
            |zero| zero + b.var - d.var
        );
...
// impl<F: Field> ConditionalEqGadget<F> for AllocatedNum<F> {
...
impl<F: Field> CondSelectGadget<F> for AllocatedNum<F> {
    fn conditionally_select<CS: ConstraintSystem<F>>(
        ...
        cs.enforce(
            ...
            |zero| zero + true_value.var - false_value.var,
            |_| cond.lc(CS::one(), F::one()),
            |zero| zero + c.var - false_value.var
        );
...
impl<F: Field> From<AllocatedNum<F>> for Num<F> {
    fn from(num: AllocatedNum<F>) -> Num<F> {
        Num {
            value: num.value,
            lc: LinearCombination::<F>::zero() + num.var,
        }
    }
}
...
impl<F: Field> Num<F> {
    ...
    pub fn add_number_with_coeff(self, variable: &AllocatedNum<F>, coeff: F) -> Self {
...
impl<F: Field> Add<&Num<F>> for Num<F> {
    type Output = Num<F>;
...
impl<F: Field> Sub<&Num<F>> for Num<F> {
    type Output = Num<F>;
