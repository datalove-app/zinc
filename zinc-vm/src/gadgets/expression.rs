...
impl<E: Engine> Expression<E> {
    pub fn new(value: Option<E::Fr>, lc: LinearCombination<E::Fr>) -> Expression<E> {
        Expression { value, lc }
    }
    pub fn constant<CS: ConstraintSystem<E::Fr>>(value: E::Fr) -> Expression<E> {
        Expression {
            value: Some(value),
    ...
    pub fn boolean<CS: ConstraintSystem<E::Fr>>(boolean_var: Boolean) -> Expression<E> {
        Expression {
            value: boolean_var.get_value_field::<E::Fr>(),
            lc: boolean_var.lc(CS::one(), E::Fr::one()),
        }
    }
    pub fn le_bits<CS: ConstraintSystem<E::Fr>>(le_bits: &[Boolean]) -> Expression<E> {
        ...
        for b in BitIterator::new(b) {
            ...
            if b {
                ...
                if current_run.len() > 0 {
                    ...
                    last_run = Some(kary_and::<E, Namespace<'_, E::Fr, CS::Root>>(
...
impl<E: Engine> From<&AllocatedNum<E>> for Expression<E> {
    fn from(num: &AllocatedNum<E>) -> Expression<E> {
        Expression {
            value: num.get_value(),
            lc: LinearCombination::<E::Fr>::zero() + num.get_variable(),
        }
    }
}
...
impl<E: Engine> From<&AllocatedBit> for Expression<E> {
    fn from(bit: &AllocatedBit) -> Expression<E> {
        Expression {
            value: bit.get_value_field::<E::Fr>(),
            lc: LinearCombination::<E::Fr>::zero() + bit.get_variable(),
        }
    }
}
...
pub fn field_into_allocated_bits_le<CS: ConstraintSystem<F>, F: PrimeField>(
    ...
    let values = match value {
        Some(ref value) => {
            let mut field_char = BitIterator::new(F::Params::MODULUS);

            let mut tmp = Vec::with_capacity(size_bits);
...
pub fn field_into_allocated_bits_le_fixed<CS: ConstraintSystem<F>, F: PrimeField>(
    ...
    // Deconstruct in big-endian bit order
    let size = F::Params::MODULUS_BITS as usize;
    let values = match value {
        Some(ref value) => {
            let mut field_char = BitIterator::new(F::Params::MODULUS);
