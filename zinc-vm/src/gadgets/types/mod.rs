...
pub fn conditional_type_check<F, CS>(
    cs: CS,
    condition: &Scalar<F>,
    scalar: &Scalar<F>,
    ...
    CS: ConstraintSystem<F>,
{
    ...
    match scalar_type {
        ...
        ScalarType::Boolean => {
            // Check as u1 integer, then change type to Boolean
...
fn conditional_int_type_check<F, CS>(
    mut cs: CS,
    condition: &Scalar<F>,
    scalar: &Scalar<F>,
    ...
    CS: ConstraintSystem<F>,
{
    ...
    let offset_expr = if !int_type.is_signed {
        ...
        let offset_fr = utils::bigint_to_fr::<F>(&offset).expect("invalid integer type length");
